...
4. Tambahkan Environment Variables (penting — tanpa ini server **menolak start**
   ...
   | `KCI_ASAL_DIIZINKAN` | `https://<nama-proyek>.vercel.app` *(domain Vercel-mu, tanpa trailing slash)* |
   ...
   > `KCI_ASAL_DIIZINKAN` diisi. Dalam mode itu `KCI_PEPPER` wajib diisi dan
   > password dashboard harus diganti dari bawaan `admin/admin123` sebelum
   > dipakai publik.
...
health-check `/api/kesehatan`, dan deklarasi `KCI_PEPPER`,
`KCI_ADMIN_PASSWORD`, serta `KCI_ASAL_DIIZINKAN` dengan `sync: false`.
...
3. Saat diminta, isi nilai untuk variabel ber-`sync: false`:
   ...
   - `KCI_ADMIN_PASSWORD` → password pengurus dari langkah 1.
   - `KCI_ASAL_DIIZINKAN` → `https://<nama-proyek>.vercel.app` *(domain Vercel-mu, tanpa trailing slash)*.
   - `KCI_TOKEN_ADMIN` → opsional; isi manual hanya bila masih perlu token legacy.
...
5. **Tambahkan Environment Variables** (Settings → Environment):
   ...
   | `KCI_ADMIN_USER` | `admin` *(atau username pilihan)* |
   | `KCI_ADMIN_PASSWORD` | password pengurus dari langkah 1 |
   | `KCI_ASAL_DIIZINKAN` | `https://<nama-proyek>.vercel.app` *(ganti dengan domain Vercel-mu, tanpa trailing slash)* |
...
3. Jika memakai skema Vercel + Render, sesuaikan `vercel.json` agar `/api/*`
   diproxy ke URL Render (konfigurasi bawaan repo saat ini adalah FULL
   VERCEL, jadi langkah ini hanya untuk mode Render):
...
curl -X POST -H "X-Admin-User: $ADMIN_USER" -H "X-Token-Admin: $TOKEN" -H "X-CSRF-Token: $CSRF" $API/api/pengurus/pindai
...
- **Start gagal** (server menolak jalan) → pastikan `KCI_PEPPER` ≥ 16 karakter,
  `KCI_ADMIN_PASSWORD` sudah diganti dari `admin123`, dan `KCI_ASAL_DIIZINKAN`
  berisi domain frontend; Render berjalan sebagai `NODE_ENV=production`.